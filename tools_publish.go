...
发布是同步操作，会等待所有账号发布完成后返回结果汇总。`),
			...
			mcp.WithBoolean("headless", mcp.Description("是否使用无头浏览器模式（后台运行不显示窗口），默认 false")),
			mcp.WithBoolean("closeBrowser", mcp.Description("发布完成后是否关闭浏览器以释放资源，默认 true（推荐）；传 false 则保留浏览器以便人工查看或后续操作，长时间不关闭会占用系统资源")),
...
参数说明同 publish_article。`),
	...
	s.AddTool(
		mcp.NewTool("close_browser",
			mcp.WithDescription(`关闭所有当前已打开的浏览器实例，立即释放系统资源。

适用场景：
- 多次发布后浏览器实例堆积，占用过多内存
- 发布工具 closeBrowser 参数未设为 true，浏览器持续保留
- 怀疑存在遗留的浏览器进程需要统一清理

注意：该操作会终止所有TurboPush启动的的浏览器进程，正在进行的发布任务将自动取消。`),
		),
		func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
			resp, err := c.Post("/closeBrowser", map[string]any{})
			if err != nil {
...
func publishHandler(c *Client, pathTemplate string, contentType string) server.ToolHandlerFunc {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		body := map[string]any{
			...
			// 默认发布完成后关闭浏览器以释放资源；调用方显式传 false 时保留浏览器
			"closeBrowser": true,
		}
		...
		if v, ok := args["closeBrowser"].(bool); ok {
			body["closeBrowser"] = v
		}
